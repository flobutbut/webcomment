...
export function UserMenu({ profile, session }: { profile: Profile | null; session: Session }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 px-2 py-1.5 rounded-6 hover:bg-gray-100 dark:hover:bg-dark-hover transition-colors"
      >
        <Avatar username={username} initials={profile?.initials} avatarUrl={profile?.avatar_url} size="sm" />
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300 max-w-[120px] truncate">{username}</span>
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className="absolute right-0 top-full mt-1.5 z-20 bg-white dark:bg-dark-800 border border-gray-200 dark:border-dark-border rounded-12 shadow-lg py-1 min-w-[160px]">
            ...
            <NavLink
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-dark-hover transition-colors"
            >
